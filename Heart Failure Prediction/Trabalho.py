...
# Médicos usam creatinina como preditor de prognóstico; ótimo insight biomédico.
 ...
 # Hiponatremia (baixo sódio)
    ...
    # Calcula as médias de ejeção do coração de cada grupo (Sobreviveu / Morreu)
    media_ejec = df.groupby('DEATH_EVENT')['ejection_fraction'].mean().sort_index()

    valores = media_ejec.values
    labels = [f'Sobreviveram \n (Média de {valores[0]:.2f}%)', f'Morreram \n(Média de {valores[1]:.2f}%)']

    plt.figure(figsize=(6, 6))
    plt.bar(labels, valores, color=['#8FD19E', '#E07B91'], alpha=0.8, edgecolor='black', width=0.5)

    plt.title('Fração de Ejeção Média por Ocorrência de Morte Cardíaca', fontsize=14, pad=15)
    plt.ylabel('Fração de ejeção (%)')
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    ...
    plt.barh(correlacoes.index, correlacoes.values, color='#E07B91', alpha=0.8, edgecolor='black')
    plt.title('Correlação dos Fatores com a Mortalidade', fontsize=14, pad=15)
